...
function PropertyDataTable({
  ...
  const [selectedRowForPayment, setSelectedRowForPayment] = useState<
    string | null
  >(null);
  ...
  const tableHeaders = [
    ...
    { key: "addPaymentPlan", label: "ADD PAYMENT PLAN" },
  ...
  const toggleColumnVisibility = useCallback(
    (columnKey: string, headers?: string) => {
      if (headers) {
  ...
  const getSelectedData = useCallback(() => {
    if (isSelectionMode) {
      ...
      const visibleKeys = Object.keys(visibleColumns).filter(
  ...
  const renderPaymentPlanCell = (record: any, planKey: string) => {
    ...
    // Get the developerPrice from the first plan
    const developerPrice = paymentPlan[0]?.developerPrice || 0;
              ...
              {paymentPlan.map((plan: any, idx: number) => (
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-medium">Developer Price</span>
                    <Badge variant="secondary" className="text-xs">
                      {plan.developerPrice?.toLocaleString() || "N/A"}
                    </Badge>
                  </div>
                </div>
              ))}
        ...
        case "paymentPlan3":
          ...
          return (
            <div className="flex items-center justify-center">
              <span > {developerPrice.toLocaleString()}</span>
            </div>
          );
        case "plotSizeSqFt":
        case "BuaSqFt":
          ...
          return (
            <div className="flex justify-center">
              <Button
                ...
                onClick={() => handlePaymentPlan(record._id)}
              >
          ...
          return (
            <div className="flex justify-center">
              <Button
                ...
                className="h-8 px-2 text-green-600 border-green-200 hover:bg-green-50 hover:text-green-700"
                onClick={() =>
                  window.open(`/inventory-details/${record._id}`, "_blank")
                }
                disabled={isAttachingDocument}
              >
              ...
              <div className="flex flex-wrap justify-center items-center gap-2">
                {(selectedRows.length > 0 || selectedColumns.length > 0) && (
                  <Badge className="border-border">
                    {selectedRows.length} rows × {selectedColumns.length}{" "}
                    columns
                  </Badge>
                )}
          ...
          <div className="flex gap-2">
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={setShowHeaderCategories}
                id="show-headers"
              />
              ...
              {showHeaderCategories && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem
                      onClick={() =>
                        toggleColumnVisibility("a", "availability")
                      }
                    >
                      Availability
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() =>
                        toggleColumnVisibility("a", "unitTenancyDetails")
                      }
                    >
                    ...
                    <DropdownMenuItem
                      onClick={() =>
                        toggleColumnVisibility("a", "paymentDetails")
                      }
                    >
                      Payment Details
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() =>
                        toggleColumnVisibility("a", "paymentPlanDetails")
                      }
                    >
                      Payment Plan Details
                    </DropdownMenuItem>
                  ...
                  <TableRow>
                    ...
                    {(checkState === "projectDetails" ||
                      ...
                      <TableHead
                        ...
                        colSpan={
                          projectDetails.filter((key) => visibleColumns[key])
                            .length
                        }
                    ...
                    {(checkState === "unitDetails" || checkState === "all") && (
                      <TableHead
                        onClick={() =>
                          toggleColumnVisibility("a", "unitDetails")
                        }
                        colSpan={
                          unitDetails.filter((key) => visibleColumns[key])
                            .length
                        }
                    ...
                    {(checkState === "availability" ||
                      ...
                      <TableHead
                        ...
                        colSpan={
                          availability.filter((key) => visibleColumns[key])
                            .length
                        }
                    ...
                    {(checkState === "paymentDetails" ||
                      ...
                      <TableHead
                        ...
                        colSpan={
                          paymentDetails.filter((key) => visibleColumns[key])
                            .length
                        }
                        className="text-center cursor-pointer font-bold bg-gradient-to-b from-blue-500 to-blue-400 border-r border-border relative"
                      >
                    ...
                    {(checkState === "actions" || checkState === "all") && (
                      <TableHead
                        onClick={() => toggleColumnVisibility("a", "actions")}
                        colSpan={
                          actions.filter((key) => visibleColumns[key]).length
                        }
                    ...
                    .map((header) => (
                      <TableHead
                        ...
                        className={cn(
                          ...
                          header.key === "addPaymentPlan" && "w-[150px]",
                      ...
                      >
                        ...
                        <div
                          className={
                            isSelectionMode ? "uppercase text-xs font-bold" : ""
                          }
                        >
                          {header.label}
                        </div>
