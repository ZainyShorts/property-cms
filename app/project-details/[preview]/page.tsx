...
interface PropertyData {
  ...
  masterDevelopment: any
  subDevelopment: any
  paymentPlan1?: PaymentPlan // Change this line
  paymentPlan2?: PaymentPlan // Add this line
  paymentPlan3?: PaymentPlan // Add this line
  [key: string]: any // Add this to allow for dynamic payment plan keys
  __v: number
}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {(() => {
          // Extract payment plans from propertyData
          const paymentPlans = Object.keys(propertyData)
            .filter((key) => key.startsWith("paymentPlan"))
            .map((key) => ({
              key,
              plan: propertyData[key],
              planNumber: key.replace("paymentPlan", ""),
            }))
            .filter((item) => item.plan) // Only include plans that exist
            ...
            <Card className="mb-8 overflow-hidden border-none shadow-lg bg-white dark:bg-black">
              ...
              <CardContent className="p-4 sm:p-6">
                <div className="space-y-8">
                  {paymentPlans.map(({ key, plan, planNumber }) => (
                    <div key={key} className="border border-gray-200 dark:border-gray-800 rounded-lg p-6">
                      <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-semibold">Payment Plan {planNumber}</h3>
                        {plan.DeveloperPrice > 0 && (
                          <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-lg">
                            <Banknote className="h-5 w-5 text-primary" />
                            <span className="font-semibold text-primary">
                              Developer Price: {plan.DeveloperPrice.toLocaleString()}
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      {plan.plan && plan.plan.length > 0 ? (
                            ...
                            <table className="w-full border-collapse border border-gray-200 dark:border-gray-800 rounded-lg">
                              <thead>
                                <tr className="bg-gray-50 dark:bg-gray-900/50">
                                  ...
                                  <th className="p-3 text-right text-sm font-medium text-gray-600 dark:text-gray-300 border-b">
                                    Amount
                                  </th>
                                  <th className="p-3 text-center text-sm font-medium text-gray-600 dark:text-gray-300 border-b">
                                    Status
                              ...
                              <tbody>
                                {plan.plan.map((payment, paymentIndex) => {
                                  const paymentDate = new Date(payment.date)
                                  const isPastDue = paymentDate < new Date()
                                  const isUpcoming =
                                    paymentDate > new Date() &&
                                    paymentDate <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) // Next 30 days
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-center gap-2">
                                          <div className="w-12 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                                            <div
                                              ...
                                              style={{ width: `${Math.min(payment.constructionPercent, 100)}%` }}
                                            />
                                          </div>
                                          <span className="text-sm font-medium">{payment.constructionPercent}%</span>
                                        </div>
                                      </td>
                                      <td className="p-3 border-b border-gray-100 dark:border-gray-800 text-right font-semibold">
                                      ...
                                      <td className="p-3 border-b border-gray-100 dark:border-gray-800 text-center">
                                        {isPastDue ? (
                                          ...
                                          <Badge
                                            variant="secondary"
                                            className="text-xs bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400"
                                          >
                                            <Clock className="h-3 w-3 mr-1" />
                                            Due Soon
                                          </Badge>
                          ...
                          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                              ...
                              <span className="text-2xl font-bold text-blue-900 dark:text-blue-100">
                                {plan.plan.reduce((sum, payment) => sum + payment.amount, 0).toLocaleString()}
                              </span>
                            </div>
                            ...
                            <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="font-medium text-green-900 dark:text-green-100">Payments</span>
                              </div>
                            ...
                            <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg border border-purple-200 dark:border-purple-800">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="font-medium text-purple-900 dark:text-purple-100">Duration</span>
                              </div>
                              <span className="text-2xl font-bold text-purple-900 dark:text-purple-100">
                                {plan.plan.length > 0
                                  ? Math.ceil(
                                      (new Date(plan.plan[plan.plan.length - 1].date).getTime() -
                                        new Date(plan.plan[0].date).getTime()) /
                                        (1000 * 60 * 60 * 24 * 30),
                                    )
                                  : 0}{" "}
                                months
                        ...
                        <div className="text-center py-8 text-muted-foreground bg-gray-50/50 dark:bg-black/20 rounded-lg border border-dashed">
                          ...
                          <p className="font-medium">No payment schedule available for Plan {planNumber}</p>
                          <p className="text-sm mt-1">Contact the developer for payment details</p>
                        </div>
                      )}
                    </div>
                  ))}
